...
const TEST_CLIENT_ID = `test-legacy-client-${Date.now()}`;
const TEST_CLIENT_ID_2 = `test-legacy-client-2-${Date.now()}`;
...
describe("GET /legacy/clients/:id", () => {
  ...
  it(
    ...
    skipIfNoDb(async () => {
      const res = await app.request(`/legacy/clients/${TEST_CLIENT_ID}`, authed(), TEST_ENV);
      expect(res.status).toBe(200);
      const body = await res.json() as any;
  ...
  it(
    ...
    skipIfNoDb(async () => {
      const res = await app.request(
        `/legacy/clients/${TEST_CLIENT_ID}?include=google_credentials`,
        authed(),
        TEST_ENV
      );
      expect(res.status).toBe(200);
  ...
  it(
    ...
    skipIfNoDb(async () => {
      const res = await app.request(
        `/legacy/clients/${TEST_CLIENT_ID}?include=slack_credentials`,
        authed(),
        TEST_ENV
      );
      expect(res.status).toBe(200);
  ...
  it(
    ...
    skipIfNoDb(async () => {
      const res = await app.request(
        `/legacy/clients/${TEST_CLIENT_ID}?include=google_credentials,slack_credentials`,
        authed(),
        TEST_ENV
      );
      expect(res.status).toBe(200);
  ...
  it(
    ...
    skipIfNoDb(async () => {
      const res = await app.request(
        `/legacy/clients/${TEST_CLIENT_ID_2}`,
        authed(),
        TEST_ENV
      );
      expect(res.status).toBe(404);
...
describe("GET /legacy/clients", () => {
  it(
    "returns 200 with only active clients",
    skipIfNoDb(async () => {
      const res = await app.request("/legacy/clients", authed(), TEST_ENV);
      expect(res.status).toBe(200);
      const body = await res.json() as any;
      expect(body.success).toBe(true);
  ...
  it(
    ...
    skipIfNoDb(async () => {
      const res = await app.request("/legacy/clients?limit=1", authed(), TEST_ENV);
      expect(res.status).toBe(200);
      const body = await res.json() as any;
  ...
  it("returns 401 without X-API-Key", async () => {
    const res = await app.request("/legacy/clients", {}, TEST_ENV);
    expect(res.status).toBe(401);
  });
...
const NEW_CLIENT_ID = `test-legacy-new-${Date.now()}`;

describe("POST /legacy/clients", () => {
  afterAll(async () => {
    if (!DB_URL) return;
    const sql = neon(DB_URL);
  ...
  it(
    ...
    skipIfNoDb(async () => {
      ...
      const getRes = await app.request(
        `/legacy/clients/${id}?include=google_credentials`,
        authed(),
        TEST_ENV
      );
...
// ─── PATCH /legacy/clients/:id ────────────────────────────────────────────────

describe("PATCH /legacy/clients/:id", () => {
  it(
